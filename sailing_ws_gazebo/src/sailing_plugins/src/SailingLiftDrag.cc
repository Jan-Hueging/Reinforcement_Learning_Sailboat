...
class gz::sim::systems::SailingLiftDragPrivate
{
  ...
  public: gz::math::Vector3d windVelocity = gz::math::Vector3d::Zero;

  /// Topic, auf dem Wind empfangen wird
  public: std::string windTopic = "/world/default/wind";

  /// Transport Node
  public: gz::transport::Node node;


  /// Callback für Windnachrichten
  public: void OnWindMsg(const gz::msgs::Vector3d &_msg);
...
//////////////////////////////////////////////////
void SailingLiftDragPrivate::Load(const EntityComponentManager &_ecm,
                           const sdf::ElementPtr &_sdf)
{
  if (!this->model.Valid(_ecm))
  {
    gzerr << "The SailingLiftDrag system should be attached to a model entity. "
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (entities.empty())
    {
      gzerr << "Link with name[" << linkName << "] not found. "
             << "The SailingLiftDrag will not generate forces\n";
  ...
  {
    gzerr << "The SailingLiftDrag system requires the 'link_name' parameter\n";
    this->validConfig = false;
    return;
  }
  ...
  /*
  if (_sdf->HasElement("wind_velocity"))
  {
   this->windVelocity =
    _sdf->Get<math::Vector3d>("wind_velocity", this->windVelocity).first;
  }
  */
  ...
  this->windTopic =
    _sdf->Get<std::string>(
        "wind_topic",
        this->windTopic).first;

  this->node.Subscribe(
    this->windTopic,
    &SailingLiftDragPrivate::OnWindMsg,
    this);
...
void SailingLiftDragPrivate::OnWindMsg(
...
//////////////////////////////////////////////////
SailingLiftDrag::SailingLiftDrag()
    : System(), dataPtr(std::make_unique<SailingLiftDragPrivate>())
{
}

//////////////////////////////////////////////////
void SailingLiftDragPrivate::Update(EntityComponentManager &_ecm)
{
  GZ_PROFILE("SailingLiftDragPrivate::Update");
...
//////////////////////////////////////////////////
void SailingLiftDrag::Configure(const Entity &_entity,
...
//////////////////////////////////////////////////
void SailingLiftDrag::PreUpdate(const UpdateInfo &_info, EntityComponentManager &_ecm)
{
  GZ_PROFILE("SailingLiftDrag::PreUpdate");
...
GZ_ADD_PLUGIN(SailingLiftDrag,
                    System,
                    SailingLiftDrag::ISystemConfigure,
                    SailingLiftDrag::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(SailingLiftDrag, "gz::sim::systems::SailingLiftDrag")